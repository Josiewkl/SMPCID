...
DATASETS = Registry('dataset') #创建一个名为dataset的注册表
...
def build_dataset(cfg, default_args=None):  #通过调用 build_from_cfg 创建
    ...
    if isinstance(cfg, (list, tuple)):
        ...
        dataset = RepeatDataset(
            build_dataset(cfg['dataset'], default_args), cfg['times']) #cfg['times'] 用于指定对数据集的重复采样次数，即在训练过程中对数据集进行多次遍历以提高训练效果
    elif cfg['type'] == 'ClassBalancedDataset':
        dataset = ClassBalancedDataset(
            build_dataset(cfg['dataset'], default_args), cfg['oversample_thr']) #default_args是默认的参数;cfg['oversample_thr']用于设置采样的阈值;cfg['oversample_thr']对数据进行采样处理，最后返回采样后的数据集
