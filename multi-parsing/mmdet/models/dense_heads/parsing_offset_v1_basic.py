...
class Parsing_offset_v1_basic(nn.Module):
    ...
    def forward(self, feats, eval=False):
        ...
        new_feats = self.split_feats(feats) #将feats分离为不同的特征层
        featmap_sizes = [featmap.size()[-2:] for featmap in new_feats]
        upsampled_size = (featmap_sizes[0][0] * 2, featmap_sizes[0][1] * 2)  #size(288,192)
        cate_pred, kernel_pred = multi_apply(self.forward_single, new_feats, #multi_apply函数用于将forward_single函数应用于多个特征层
                                                       list(range(len(self.seg_num_grids))),
                                                       eval=eval, upsampled_size=upsampled_size)
        if self.enable_cate_decouple == True: #未启用
        ...
        new_feats = self.concate_feats(feats) #将所有特征层拼接在一起，送入中心分支、偏移分支、分割分支和关键点分支进行处理
        ...
        if self.enable_segment == True: #未启用
            segment_pred = self.forward_segment(mixed_feats)
        else:
            segment_pred = None
        
        if self.enable_keypoints == True:#未启用
    ...
    def forward_offset(self, x_in): #偏移量回归

        offset_feats = x_in 
        # resize to grids
        seg_num_grid = self.seg_num_grids[0]
        offset_feats = F.interpolate(offset_feats, size=seg_num_grid, mode='bilinear',align_corners=True) #将x_in插值到指定的网格大小
        ...
        offset_feats = torch.cat([offset_feats, coord_feat], 1) #与坐标特征拼接起来

        # reduce dimension by 1*1 conv使用1x1卷积降维
        offset_feature = self.offset_convs(offset_feats)

        # decoupled 58 child branches
        final_offset = []
        for j in range(self.cate_out_channels):
            final_offset.append( #每个分支使用一个卷积块和一个最终的卷积层来生成偏移量
                self.offset_final[j](
    ...
    def forward_center(self, x, eval=False): #通过给定的输入张量x预测中心点的热图
        ...
        coord_feat = torch.cat([x, y], 1)
        center_feat = torch.cat([center_feat, coord_feat], 1) #torch.linspace函数生成一组网格坐标点，并将其与center_feat连接起来形成新的特征图center_feat
        ...
        center_feat = F.interpolate(center_feat, size=seg_num_grid, mode='bilinear', align_corners=True)#将center_feat通过插值操作调整到所需的网格大小，即seg_num_grid

        # get center heatmap
        center_feat = center_feat.contiguous()
        for i, center_layer in enumerate(self.center_convs):
            center_feat = center_layer(center_feat)
        center_pred = self.center_heatmap(center_feat) #对center_feat进行特征提取，并将其传递给一个最终输出通道数为1的卷积层center_heatmap，得到中心点热图center_pred

        if eval:
            center_pred = points_nms(center_pred.sigmoid(), kernel=2).permute(0, 2, 3, 1) #对center_pre进行非极大值抑制处理
    ...
    def forward_single(self, x, idx, eval=False, upsampled_size=None): #用于语义分割
        ins_kernel_feat = x
        # ins branch
        # concat coord
        x_range = torch.linspace(-1, 1, ins_kernel_feat.shape[-1], device=ins_kernel_feat.device)
