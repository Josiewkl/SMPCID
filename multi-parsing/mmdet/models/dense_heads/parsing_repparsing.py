...
class Repparsing_Head(nn.Module):
    ...
    def forward_kernel(self, x):
        ...
        # reduce dimension by 1*1 conv
        kernel_feat = self.segment_converts(x)
    ...
    def forward_instance(self, x):
        instance_feat = self.dekr_convs(x)
        # ins branch
        # concat coord
    ...
    def forward_semantic(self, x):
        semantic_feat = self.segment_converts(x)
        # semantic branch
        # concat coord
    ...
    def loss(self,
        ...
        if self.enable_semantic:
            ...
            loss_semantic_pos = dice_loss(semantic_preds[parsing_index], gt_parsings[parsing_index])
        ...
        if self.enable_fusion_multi:
            ...
            sem_preds = F.conv2d(F.normalize(sem_preds,p=2,dim=1), F.normalize(semantic_kernels,p=2,dim=1), stride=1)
            par_segments_pos = []
            par_segments_neg = []
            for img_segment, sem_pred, parsing_label, ins_ind_label in zip(img_segments, sem_preds, parsing_label_list, ins_ind_label_list):
            ...
            for img_i in range(ins_kernel_map.shape[0]):
                ...
                for cate_i in cate_list:
                    ...
                    parsing_kernels.append(torch.cat([ins_kernel[ins_i], self.seg_kernels[cate_i-1]], 0).unsqueeze(0))
                    former_cate = cate_i
            ...
            fusion_preds = torch.cat([instance_preds, sem_preds], 1)
    ...
    def get_parsing_single(self,
        ...
        if not infer_by_pixel:
            ...
            for i_ins in range(len(ins_masks[0])):
                ...
                cate_labels = torch.from_numpy(np.array([i for i in range(58)])).cuda()
                #_, labels, _, keep_inds = matrix_nms((ins_pars > cfg.mask_thr).float(), cate_labels, sem_scores, filter_thr=0.3)
                labels = torch.where(sem_scores>0.3)[0]
                ...
                neg_index = ~ins_pars_index.sum(1).bool()
                ins_pars = (torch.argmax(ins_pars,1) + 1)
                ins_pars = ins_pars * ~neg_index
            ...
            vis_results = (center_preds.cpu().numpy(), ins_masks.cpu().numpy(), sem_conf.cpu().numpy())
