...
def eval_seg_ap(results_all, dat_list, nb_class=59, ovthresh_seg=0.5, Sparse=False, From_pkl=False, task_id=None):
    ...
    for d in trange(nd, desc='Finding AP^P at thres %f..'%ovthresh_seg):
        ...
        if ovmax > ovthresh_seg:
            ...
            fp_seg[d] =  1.
            if low_quality < 2000 and j == 4:
                filename = R['anno_adds'][jmax].split("/")[-1].split(".")[0]
                img = mmcv.imread("/mnt/LV-MHP-v2/val/images/" + filename.split("_")[0] + ".jpg")
                img = img * 0.5
                img[mask_pred>0] = np.array([255,255,255])
                mmcv.imwrite(img, "/mnt/SMP2.0/multi-parsing/visual/offset/bad_case/" + filename.split("_")[0] + ".jpg")
...
def eval_seg_ap_ori(results_all, dat_list, nb_class=59, ovthresh_seg=0.5, Sparse=False, From_pkl=False, task_id=None):
    '''
    From_pkl: load results from pickle files 
    Sparse: Indicate that the masks in the results are sparse matrices
    '''
