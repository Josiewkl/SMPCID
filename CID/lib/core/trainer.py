...
class Trainer(object):
    ...
    def train(self, epoch, data_loader, optimizer):
        ...
        batch_time = AverageMeter() #计时
        data_time = AverageMeter()
        multi_heatmap_loss_meter = AverageMeter() #对loss值进行记录
        single_heatmap_loss_meter = AverageMeter()
        contrastive_loss_meter = AverageMeter()

        self.model.train() #开启模型训练模式，必须先声明

        end = time.time()
        for i, batched_inputs in enumerate(data_loader): #数据迭代读取的循环函数
            data_time.update(time.time() - end)

            loss_dict = self.model(batched_inputs) #进入CID的forward函数
