...
class CID(nn.Module): #类继承
    def __init__(self, cfg, is_train):
        super().__init__()
        self.device = torch.device(cfg.MODEL.DEVICE) #确定用GPU还是CPU
    ...
    def forward(self, batch_inputs):
        images = [x['image'].unsqueeze(0).to(self.device) for x in batch_inputs] #按顺序取出图片
        images = torch.cat(images, dim=0) #concat将图片进行拼接

        feats = self.backbone(images) #将图片输入backbone

        if self.training:
            multi_heatmap_loss, contrastive_loss, instances = self.iia(feats, batch_inputs) #输入到IIA
